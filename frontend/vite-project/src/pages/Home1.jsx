...
export default function Home1() {
  ...
  const [lang] = useState(localStorage.getItem("app_lang") || "vi");
  ...
  // --- LOGIC TÌM PHIM XỊN NHẤT ---
  const topRatedMovie = useMemo(() => {
    if (!movies || movies.length === 0) return null;
    return [...movies].sort((a, b) => (b.rating || 0) - (a.rating || 0))[0];
  }, [movies]);

  // --- LOGIC HIỂN THỊ POSTER (ĐÃ SỬA: THEO USER ID) ---
  useEffect(() => {
    // Chỉ hiện khi đã xác thực xong, có user và đã load xong phim
    if (!isCheckingAuth && authUser && topRatedMovie) {
      // Mỗi user sẽ có một chìa khóa riêng dựa trên ID của họ
      const userKey = `luna_seen_intro_${authUser._id}`;
      const hasSeenIntro = localStorage.getItem(userKey);
      
      if (!hasSeenIntro) {
        setShowIntro(true);
      }
    }
  }, [authUser, isCheckingAuth, topRatedMovie]);

  const closeIntro = () => {
    setShowIntro(false);
    if (authUser?._id) {
      // Đánh dấu là user cụ thể này đã xem poster
      localStorage.setItem(`luna_seen_intro_${authUser._id}`, "true");
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 pb-12 border-b border-zinc-700">
            ...
            <div>
              ...
              <ul className="space-y-3 text-[11px] font-bold uppercase tracking-tight">
                <li onClick={() => navigate("/movies", { state: { filter: "showing" } })} className="hover:text-amber-400 cursor-pointer flex items-center gap-2"><ChevronRight size={12} /> Phim đang chiếu</li>
                <li onClick={() => navigate("/movies", { state: { filter: "coming" } })} className="hover:text-amber-400 cursor-pointer flex items-center gap-2"><ChevronRight size={12} /> Phim sắp chiếu</li>
            ...
            <div>
              ...
              <ul className="space-y-3 text-[11px] font-bold uppercase tracking-tight">
                <li onClick={() => navigate("/privacy-policy")} className="hover:text-amber-400 cursor-pointer transition-colors">Chính sách bảo mật</li>
                <li onClick={() => navigate("/customer-care")} className="hover:text-amber-400 cursor-pointer transition-colors">Chăm sóc khách hàng</li>
                <li onClick={() => navigate("/faq")} className="hover:text-amber-400 cursor-pointer transition-colors">Câu hỏi thường gặp</li>
              </ul>
            </div>
            <div>
